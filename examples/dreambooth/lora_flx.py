...
def create_network_from_weights(multiplier, file, unet, weights_sd=None, for_inference=False, **kwargs):
    ...
    network = LoRANetwork(
        unet, multiplier=multiplier, modules_dim=modules_dim, modules_alpha=modules_alpha, module_class=module_class
    )
...
class LoRANetwork(torch.nn.Module):
    ...
    LORA_PREFIX_UNET = "lora_unet" #"lora_transformer" 
                        ...
                        if "blocks_6" in lora_name or "blocks_18" in lora_name or "blocks_9" in lora_name or "blocks_25" in lora_name:
                                if not "attn" in lora_name and not "proj_mlp" in lora_name and not "ff_" in lora_name and not "norm" in lora_name:
                                    lora = module_class(
                                        ...
                                        self.multiplier,
                                        dim,
                                        alpha*1.4 if "blocks_25" in lora_name else alpha,
                                        dropout=dropout,
                                        rank_dropout=rank_dropout,
                                        module_dropout=module_dropout,
                                    )
    ...
    def merge_to(self, unet, weights_sd, dtype, device):
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        def convert(std):
            ...
            for key, value in std.items():
                ...
                for i in range(40):
                    ...
                    new_key = new_key.replace(f'{i}_proj', f'{i}.proj')
                    new_key = new_key.replace(f'net_{i}_', f'net.{i}')
                    new_key = new_key.replace(f'blocks_{i}_', f'blocks.{i}.')
                    new_key = new_key.replace(f'blocks_{i}', f'blocks.{i}')

                new_key = new_key.replace('lora_down', 'lora_A')
                new_key = new_key.replace('attn_', 'attn.')
                new_key = new_key.replace('_attn', '.attn')
                new_key = new_key.replace('_.', '.')
                new_key = new_key.replace('_0_', '.0')
                new_key = new_key.replace('ff_net_0', 'ff.net.0')
                new_key = new_key.replace('ff_net', 'ff.net')
                new_key = new_key.replace('_ff', '.ff')
                new_key = new_key.replace('context_net', 'context.net')
                new_key = new_key.replace('_linear', '.linear')
                new_key = new_key.replace('_0', '.0')
                new_key = new_key.replace('_norm', '.norm')
                new_key = new_key.replace('lora_up', 'lora_B')
                new_key = new_key.replace('_lora', '.lora')
                new_key = new_key.replace('proj_.', 'proj.')
                new_key = new_key.replace('linear_.', 'linear.')
                new_key = new_key.replace('mlp_.', 'mlp.')
        ...
        state_dict = convert(self.state_dict())
